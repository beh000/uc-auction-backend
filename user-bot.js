...
function forwardPhoto(fileId, caption) {
  if (!process.env.ADMIN_BOT_TOKEN || !process.env.ADMIN_CHAT_ID) return Promise.resolve();
  const body = JSON.stringify({ chat_id: process.env.ADMIN_CHAT_ID, photo: fileId, caption, parse_mode: 'HTML' });
  const options = {
    hostname: 'api.telegram.org',
    path: `/bot${process.env.ADMIN_BOT_TOKEN}/sendPhoto`,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) }
  };
...
async function handleUpdate(update) {
  ...
  // PUBG ID from auction winner
  if (session?.step === 'waiting_winner_pubg_id' && text) {
    const adminBot = require('./admin-bot');
    adminBot.receivePubgId(userId, text, session.uc, session.prize, name);
    delete sessions[userId];
    await sendUser(chatId,
      `✅ <b>PUBG ID принят!</b>\n\n` +
      `Ваш ID: <code>${text}</code>\n` +
      `Зачислим <b>${session.uc} UC</b> в течение нескольких минут.\n\n` +
  ...
  if (msg.photo && session?.step === 'waiting_coins_screenshot') {
    ...
    // Also notify admin bot with buttons
    const adminBot = require('./admin-bot');
    adminBot.notifyAdmin(
      `👆 Чек выше. Подтвердить зачисление?`,
      [[
